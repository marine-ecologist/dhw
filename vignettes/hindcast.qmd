...
GBR_OISST2_SST_QDM <- hindcast_qm(
       input1 = GBR_OISST_SST,
       input2 = GBR_ERA5_SST,
       n_quantiles = 100,
       method = "qdm",
...
qdm_qm <- rbind(GBR_OISST2_SST_QM, GBR_OISST2_SST_QDM) %>%
  pivot_wider(names_from = dataset, values_from = mean) %>%
  mutate(year = year(date)) %>%
  rename(QM = 2, QDM = 3)
...
# reference SD (QM)
sd_ref <- sd(qdm_qm$QDM, na.rm = TRUE)

# max SD for axis scaling
sd_max <- max(c(cor_stats_qdm_qm$sd_obs+2,
                cor_stats_qdm_qm$sd_mod+2), na.rm = TRUE)
