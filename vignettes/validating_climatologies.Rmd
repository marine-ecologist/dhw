...
CoralTempSST <- rast("/Users/rof011/GBR-dhw/datasets/coraltemp/GBR_coraltemp_v3.1_sst.nc") |> crop(grid_point)
crs(CoralTempSST) <- "EPSG:4283"

CoralTempDailyClimatology <- rast("/Users/rof011/GBR-dhw/datasets/coraltemp/ct5km_climatology_v3.1.nc") |> crop(grid_point)
CoralTempDailyClimatology <- CoralTempDailyClimatology[[2:13]] # select only the MM values
crs(CoralTempDailyClimatology) <- "EPSG:4283"
...
plot2(test_SST$mm, CoralTempDailyClimatology)

```


AFAIK this difference is due to the NOAA calculation of their Daily Climatologies, which are based on the full SST Timeseries (1985-01-01 to 2012-12-31), while CoralTemp is only available 1985-06-01 to 2012-12-31.
...
calculated_mm <- calculate_monthly_mean(sst_file = CoralTempSST)
...
calibrated_plot_BAA <- ggplot() + theme_bw() +
  geom_point(data=combined_data, aes(NOAA_DHW, calculated_DHW), color="purple") +
  geom_abline()

(calibrated_plot_SST | calibrated_plot_SSTA) / 
  (calibrated_plot_HS | calibrated_plot_DHW) / 
  (calibrated_plot_BAA | wrap_elements(grid::textGrob('')))
```
